...
#import "NPBeaconFMDBConstants.h"
#import "FMDatabase.h"
...
@implementation NPBeaconFMDBAdapter
...
- (id)initWithBuilding:(TYBuilding *)building
{
    self = [super init];
    if (self) {
        NSString *dbPath = [NPLocationFileManager getBeaconDBPath:building];
...
- (BOOL)insertNephogramBeaconWithUUID:(NSString *)uuid Major:(NSNumber *)major Minor:(NSNumber *)minor X:(double)x Y:(double)y Z:(double)z Floor:(int)f ShopGid:(NSString *)shopid
{
    ...
    [arguments addObject:[NPPointConverter dataFromX:x Y:y Z:z]];
...
- (NSArray *)getAllNephogramBeacons
{
    ...
    while ([rs next]) {
        ...
        NPPublicBeacon *beacon = [NPPublicBeacon beaconWithUUID:uuid Major:@(major) Minor:@(minor) Tag:tag Location:location ShopGid:nil];
...
- (NPPublicBeacon *)getNephogramBeaconWithMajor:(NSNumber *)major Minor:(NSNumber *)minor
{
    ...
    if ([rs next]) {
        ...
        beacon = [NPPublicBeacon beaconWithUUID:uuid Major:major Minor:minor Tag:nil Location:location ShopGid:shopId];
    }
