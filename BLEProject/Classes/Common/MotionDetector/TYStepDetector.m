...
#import "TYMovingAverageTD.h"
#import "TYCumulativeSingalPowerTD.h"
...
@interface TYStepDetector()
{
    float _MAValues[MAVALUE_LENGTH];
    NSMutableArray *_MAFilters;
    TYCumulativeSingalPowerTD *_csp_Filter;
...
@implementation TYStepDetector

+ (TYStepDetector *)newDefaultStepDetector
{
    return [[TYStepDetector alloc] initWithWindow1:MA1_WINDOW Window2:MA2_WINDOW PowerCutoff:POWER_CUTOFF_VALUE];
}
...
- (id)initWithWindow1:(double)w1 Window2:(double)w2 PowerCutoff:(double)pc
{
    ...
    if (self) {
        ...
        [_MAFilters addObject:[[TYMovingAverageTD alloc] initWithWindow:_WindowMA1]];
        [_MAFilters addObject:[[TYMovingAverageTD alloc] initWithWindow:_WindowMA1]];
        [_MAFilters addObject:[[TYMovingAverageTD alloc] initWithWindow:_WindowMA2]];
        
        _csp_Filter = [[TYCumulativeSingalPowerTD alloc] init];
...
-(void)pushSensorValue:(CMAcceleration) accer AtInterval:(CFTimeInterval) timeStamp{
    ...
    if (_StepDetected && !_SingalPowerCutoff) {
        ...
        if (self.delegate && [self.delegate respondsToSelector:@selector(stepDetector:onStepEvent:)]) {
            [self.delegate stepDetector:self onStepEvent:[TYStepEvent newStepWithProbability:1.0 Duration:_strideDuration]];
        }
    }
}
...
- (void)reset
{
    ...
    for(TYMovingAverageTD *mvgTD in _MAFilters)
    {
        [mvgTD reset];
    }
